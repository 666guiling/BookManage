    ...
    function btn(respBean){
        ...
        for (let i = 0;i < respBean.data.list.length;i++){
            ...
            tr = "<tr>" +
                ...
                "<input class='btn btn-default' type='button' value='借阅' onclick='borrow(" + respBean.data.list[i].book.id + ")'>" +
                "</td>" +
                "</tr>";
            $("#tbody").append(tr);
        }
...
function btn1(respBean){
    ...
    for (let i = 0;i < respBean.data.length;i++){
        ...
        tr = "<tr>" +
            ...
            "<td>" +
            //点击编辑发送请求把员工 id 存放到 session ,响应回前端,前端跳转到修改页面,修改页面再发送ajax请求根据 id 获取 对象信息
            "<input class='btn btn-default' type='button' value='编辑' onclick='borrow(" + respBean.data[i].book.id + ")'>" +
            "</tr>";
        $("#tbody").append(tr);
    }
}
